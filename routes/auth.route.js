...
router.post( //так приложение отвечает на post-запрос
    '/register',
    [
        check("email","Некоректный email").isEmail(), //проверка emal,в случае чего,выдается оишбка из 2-го аргумента.IsEmail - встроенный валидатор в либе express-validatorб
    ...
    try{
        ...
        if(!errors.isEmpty()){
            return res.status(400).json({
                errors:errors.array(),
                message:"Некорректные данны при регистрации"
            })
        }
...
router.post('/login', 
    ...
    try{
        ...
        if(!errors.isEmpty()){
            return res.status(400).json({
                errors:errors.array(),
                message:"Некорректные данны при авторизации"
            })
        }
    const {email,password} = req.body
